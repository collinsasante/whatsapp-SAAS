 ...
 * number id is given -- so a campaign send always goes out from the exact
 * number it's assigned to -- falling back to the tenant's legacy default
 * fields (always stored plaintext, see WhatsAppNumbersService.
 * syncDefaultToTenant) only when no number is specified at all. Returns
 * null when nothing usable was found, matching this file's callers'
 * existing "skip if not configured" behavior.
 */
