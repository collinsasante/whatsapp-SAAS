...
export class OrdersService {
  ...
  async submitForPayment(tenantId: string, orderId: string, customerEmail?: string, opts?: { dryRun?: boolean }) {
    ...
    const { gatewayReference, authorizationUrl, accessCode } = opts?.dryRun
      ? { gatewayReference: `EVAL-${crypto.randomUUID()}`, authorizationUrl: null as string | null, accessCode: null as string | null }
