...
function buildService(deps: ReturnType<typeof buildDeps>) {
  ...
  const service = new ToolRegistryService(deps.products as any, deps.orders as any, deps.internalTasks as any);
  service.onModuleInit();
  return service;
}

describe('ToolRegistryService', () => {
  it('registers all six commerce/catalogue/task tools on init', () => {
  ...
  describe('execute', () => {
    it('dispatches to the matching tool and returns its result', async () => {
      ...
      expect(result).toEqual([{ id: 'p1', name: 'Wallet', priceMajorUnits: 90, currency: 'GHS', inStock: true }]);
    });
    ...
    it('catches a handler throwing and returns an error object instead of propagating', async () => {
      const deps = buildDeps();
      deps.products.findAll.mockRejectedValue(new Error('db down'));
      const service = buildService(deps);
