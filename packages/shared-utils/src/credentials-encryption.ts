...
/**
 * Pure crypto functions, deliberately free of any framework DI so the
 * backend's CredentialsEncryptionService (key resolved via ConfigService),
 * the worker's WhatsApp send processors (key resolved via process.env
 * directly, no NestJS context available there), and
 * scripts/encrypt-existing-credentials.ts (a standalone one-off backfill
 * script) all call the exact same encrypt/decrypt implementation -- never
 * worth risking independent AES-GCM implementations drifting apart across
 * apps that both read/write WhatsAppNumber.accessToken.
 */
